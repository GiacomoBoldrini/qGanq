...
def make_generator_model():
    ...
    assert model.output_shape == (None, 60, 60, 32)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    
    model.add(layers.Conv2DTranspose(16, (5, 5), strides=(2, 2), padding='same', use_bias=False))
    assert model.output_shape == (None, 120, 120, 16)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
...
def train(dataset, epochs):
  for epoch in tqdm(range(epochs)):
    ...
    for image_batch in dataset:
      train_step(image_batch, BATCH_SIZE, noise_dim, noise)
...
train_images = np.load("./data_general/events_im.npy")
train_images = train_images.reshape(train_images.shape[0], 240, 240, 1).astype('float32')
